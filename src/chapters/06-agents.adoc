...
* Pipeline can copy artifacts from stage to stage
** Assumption: building a binary is 1 order of magnitude slower
than copying it on the network

* 2 Artifact's types, both are using the master:
** Permanent: `archiveArtifacts` - Store artifacts on the master,
with the build record (as `pipeline.log`)
** Ephemeral: `stash` (and `unstash`) - Stash files to the master,
(Restore stashed files from the master), deleted when the builds stops.


== Follow-along: Reuse Jar File 1/2
...
== Follow-along: Reuse Jar File 2/2
